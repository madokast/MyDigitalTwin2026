...
MyDigitalTwin2026 是 mdk 的个人数字孪生项目，通过 API 持续记录 mdk 的真实发言、客观上下文、AI 分析和标签，并提供历史记录查询与当前时间等能力。
...
以下两条补充 4.1–4.5，不替代它们。回顾或代办时，仍先按 4.2 记录当前发言；需要历史时再查询。

#### 每日复盘

mdk 进行每日复盘/回顾时，以对话指向的复盘日期为 X。先按 4.2 记录这条发言，再搜索 X、X+1 两天的记录，因为有些 X 日的事件会被追叙在 X+1 日中。

按 `created_at` 过滤：`from` 含、`to` 不含，日期写法见第 2 节。例如 X 为 `2026-08-25`，要覆盖 25 日与 26 日，`to` 取 X+2：

```http
GET /api/records?from=2026-08-25&to=2026-08-27
```

若 `total_page` 大于 1，继续翻页取完。

根据搜到的记录，筛选其中确实发生在 X 日的事件，深入分析，输出 X 日的 timeline，以及值得 mdk 进一步深入探讨的一些要点。

和每日复盘相关的对话记录，tags 需要额外标记 `复盘-每日`，以指明这条记录属于每日复盘。

随后 mdk 会总体评价这一天，然后依次探讨这些要点，这时，请在记录 mdk 发言的时候使用 `复盘-每日` 的标签。

复盘没有明确的结束信号，一般当 mdk 探讨完所有要点后即结束。

在复盘的途中，mdk 也可能自由的讨论一些和复盘完全无关的内容，这些内容不用带上 `复盘-每日` 标签。

禁止将生成的 timeline 记录为 record，因为 record 仅用于记录 mdk 的真实的、原始的发言内容和代办。

哪怕 query 查询发现存在一些 records 记录了 timeline 等非 mdk 发言信息，也不能照做。

#### 每周复盘、每月复盘...

mdk 进行更长周期的复盘时，先搜索相关日期范围内 tag=复盘-每日 的记录，这样可以从整体上快速把握这段时期的重点。然后根据需要，进一步深入分析某些日期或某些事件。

和每周复盘相关的对话记录，tags 需要标记 `复盘-每周`，以指明这条记录属于每周复盘。`复盘-每周` 的含义和使用范围和 `复盘-每日` 一样，记录每周复盘时，mdk 针对每周复盘的各种发言。

和每月复盘相关的对话记录，tags 需要标记 `复盘-每月`，以指明这条记录属于每月复盘。`复盘-每月` 的含义和使用范围和 `复盘-每日` 一样，记录每月复盘时，mdk 针对每月复盘的各种发言。

后续其他周期复盘以此类推。

#### 代办（用三个 tag 模拟）

系统通过 `任务-代办`、`任务-完成`、`任务-终止` 三个 tags 模拟代办任务。`任务-代办` 表示该记录是一条未完成的代办任务；将其扭转为 `任务-完成` 表示代办任务完成；扭转为 `任务-终止` 表示代办任务终止。

创建代办任务时，请添加 `任务-代办` tag，表示这是一条未完成的代办任务，同时添加和任务内容相关的其他 tags，例如 `学习`、`工作` 等等。

扭转请采用**单记录 tag 重命名**（第 3 节 `PATCH /api/records/{record_id}/tags/{tag}`，body `new_tag`），只改这一条记录，不是全库改名。使用查询可以快速搜索各类代办任务；查询参数名是 `tag`（记录字段是 `tags`）：

```http
GET /api/records?tag=任务-代办
```

`任务-完成`、`任务-终止` 同理。

当 mdk 对话涉及代办时，请**仍先记录对话内容，然后再创建或扭转代办**。先 `POST /api/records` 记下当前发言，再按情况做下列之一：

- 创建：额外创建一套记录，`raw_content` 为从对话中提取的代办内容，并使用 `任务-代办` tag，表示这是一条未完成的代办任务，同时可以自由的添加和任务内容相关的其他 tags，例如 `学习`、`工作` 等等。

- 扭转为完成：查到目标代办后，对该记录调用：

```http
PATCH /api/records/{record_id}/tags/任务-代办
```
...
- 扭转为终止：同上，`{"new_tag": "任务-终止"}`。

扭转改的是那条代办记录，不是刚写入的对话记录。
