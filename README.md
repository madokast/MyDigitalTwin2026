...
mdk 进行每日回顾/复盘时，以对话指向的复盘日期为 X。先按 4.2 记录这条发言，再搜索 X、X+1 两天的记录，因为有些 X 日的事件会被追叙在 X+1 日中。
...
- 创建：对刚写入的那条记录附加 `任务-代办`：

```http
PUT /api/records/{record_id}/tags/任务-代办
```
